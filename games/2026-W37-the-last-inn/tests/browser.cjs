...
(async()=>{
 ...
 try{
  ...
  await page.locator('#decide').click();assert.match(await page.locator('#ending').innerText(),/guess in the dark/);
  await page.locator('#restart').click();
  ...
  responseMode='ok';await fullLoop();await page.locator('#suspect').selectOption('silas');await page.locator('#choice').selectOption('rescue');await page.locator('#decide').click();
  assert.match(await page.locator('#ending').innerText(),/Truth & mercy/);
