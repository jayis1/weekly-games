...
4. Select a witness, ask a question without evidence to earn trust, then present a matching discovered clue and ask about it. The provider supplies the character’s speech; the engine unlocks fixed, confirmed testimony when the trust/evidence requirements are met. The notebook’s investigation checklist marks each clue and testimony as it is secured. Every interactive NPC uses the same configured provider, with a distinct personality and separate bounded dialogue history.
5. Gather all three testimonies, compare them in the notebook, choose who silenced the bell and what to do about the courier, then **Review final decision** before the twelve beats expire. Check the selected suspect, rescue choice and testimony count; **Keep investigating** or Escape returns without spending beats. Only **Confirm final decision** ends the night. Reviewing, cancelling and confirming never contact your AI provider.
